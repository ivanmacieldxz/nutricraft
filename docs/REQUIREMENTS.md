...
- **Sección de Exploración General:**
  - Buscador global por texto libre con scroll infinito (paginación automática).
  - Filtros por categoría (Desayuno, Pasta, Vegano, etc.) y área geográfica.
  - Tarjetas de recetas con vista previa (imagen, título, categoría/s, badges de compatibilidad dietética).

### 2. Detalle de Receta (`/recipes/:id`)
- Vista completa del plato: imagen, ingredientes con medidas e instrucciones paso a paso.
- Información nutricional estimada agregada por porción (calculada mediante CalorieNinjas API).
- **Acciones Rápidas:**
...
- Sistema de validación visual de preferencias:
  - Las recetas se evalúan contra las preferencias del usuario mostrando badges informativos (verdes) o destructivos (rojos) si no son compatibles.
  - Al interactuar (guardar o agregar al plan) con una receta que contiene alergias registradas, se emite una advertencia al usuario, bloqueando la acción temporalmente.
